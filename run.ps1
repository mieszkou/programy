...
function Get-File {
    ...
    function Show-Progress {
        param ([Parameter(Mandatory)][Single]$TotalValue, [Parameter(Mandatory)][Single]$CurrentValue, [Parameter(Mandatory)][string]$ProgressText, [Parameter()][int]$BarSize = 40, [Parameter()][switch]$Complete)
    ...
    try {
        ...
        if ($CustomHeaders) {
            foreach ($header in $CustomHeaders.GetEnumerator()) {
                switch ($header.Key.ToLowerInvariant()) {
                    'referer' { $request.Referer = $header.Value; break }
                    'user-agent' { $request.UserAgent = $header.Value; break }
                    'accept' { $request.Accept = $header.Value; break }
                    'content-type' { $request.ContentType = $header.Value; break }
                    default { $request.Headers[$header.Key] = $header.Value }
                }
            }
        }
        ...
        do {
            ...
            $total += $count
            if ($fullSize -gt 0) { Show-Progress -TotalValue $fullSize -CurrentValue $total -ProgressText "- Pobieranie: " }
        } while ($count -gt 0)
    }
    finally {
        $reader.Close()
...
function InstallPcPos {
    ...
    $installerPath = Join-Path $installPath (Split-Path $uri -Leaf)
    Get-File -Uri $uri -OutFile $installerPath
    Start-Process -FilePath $installerPath -Verb RunAs -Wait -ArgumentList "--mode unattended --unattendedmodeui minimalWithDialogs --enable-components POSY --installer-language pl --TYPDBINSTALL 1 --TYPDRUK SpoofFiscalPrinter"
    # Remove-Item $installerPath    
}
